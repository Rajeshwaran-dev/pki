...
import { Button, Avatar, Select, Space, Modal, Form, Input, Row, Col, Progress, Checkbox } from 'antd';
import {
  ArrowLeftOutlined,
  EditOutlined,
  PlusOutlined,
...
const ProjectDetailPage = () => {
  ...
  const isFinancialCompact = useIsMobile(1480);
  ...
  const [expandedQuoteId, setExpandedQuoteId] = useState(INITIAL_QUOTES[0]?.id || null);
  ...
  const renderQuotes = () => (
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <div style={{ ...card, padding: 18 }}>
              ...
              <div key={record.id} style={{ ...softCard, padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'grid', justifyItems: 'end', gap: 10 }}>
                    ...
                    <Space wrap size={[6, 6]}>
                      <Button size="small" onClick={() => setExpandedQuoteId(isExpanded ? null : record.id)} style={secondaryButtonStyle}>
                        {isExpanded ? 'Hide Details' : 'Show Details'}
                      </Button>
                      <Button size="small" onClick={() => openQuoteCrudModal('edit', record)} style={secondaryButtonStyle}>Edit</Button>
                      <Button
                        size="small"
                        danger
        ...
        <Modal
          title={quoteModal.mode === 'create' ? 'Add Quotation' : quoteModal.mode === 'edit' ? 'Edit Quotation' : 'Quotation Details'}
          ...
          onOk={quoteModal.mode === 'view' ? undefined : submitQuoteModal}
          okText={quoteModal.mode === 'edit' ? 'Update Quotation' : 'Create Quotation'}
          footer={quoteModal.mode === 'view' ? [
            <Button key="close" onClick={() => {
              setQuoteModal({ open: false, mode: 'create', record: null });
              quoteForm.resetFields();
            }}>
          ...
          width={760}
        >
          <Form form={quoteForm} layout="vertical" disabled={quoteModal.mode === 'view'}>
            <Row gutter={12}>
              <Col xs={24} sm={8}>
                <Form.Item name="quoteNo" label="Quote No" rules={[{ required: true, message: 'Quote number is required' }]}>
                  <Input />
                </Form.Item>
              </Col>
              <Col xs={24} sm={8}>
              ...
              <Col xs={24} sm={8}>
                <Form.Item name="validUntil" label="Valid Until" rules={[{ required: true, message: 'Valid until date is required' }]}>
                  <Input type="date" />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={12}>
              <Col xs={24} sm={12}>
                <Form.Item name="status" label="Status" rules={[{ required: true, message: 'Status is required' }]}>
                  <Select options={QUOTE_STATUSES.map(value => ({ value, label: value }))} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={12}>
                <Form.Item name="amount" label="Approved Amount" rules={[{ required: true, message: 'Amount is required' }]}>
                  <Input type="number" />
                </Form.Item>
              </Col>
            </Row>
            <Form.Item name="summary" label="Proposal Summary" rules={[{ required: true, message: 'Summary is required' }]}>
              <Input.TextArea rows={5} />
            </Form.Item>
          </Form>
        </Modal>
  ...
  const renderActivities = () => {
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isCompact ? '1fr' : 'minmax(0, 1.25fr) 280px', gap: 16 }}>
          <div
            style={{
              ...card,
              padding: 20,
              background: isDark
                ? 'linear-gradient(135deg, rgba(8,27,47,1) 0%, rgba(18,55,84,1) 100%)'
                : 'linear-gradient(135deg, #fff7ef 0%, #ffffff 100%)',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 18, color: 'var(--text)' }}>Activity Execution Board</div>
                <div style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 4 }}>
                  Track planned work, live execution, and completion targets for this project.
                </div>
              </div>
              <BadgePill label={`${activityProgressPercent}% complete`} tone="accent" />
            </div>
            <Progress percent={activityProgressPercent} strokeColor="#D69F6D" showInfo={false} style={{ marginBottom: 16 }} />

            <div style={{ display: 'grid', gridTemplateColumns: detailSummaryGridColumns, gap: 12 }}>
              {[
                {
                  label: 'Next Scheduled Work',
                  value: upcomingActivity ? upcomingActivity.orderId : 'No activity',
                  helper: upcomingActivity ? formatDisplayDate(upcomingActivity.startDate) : 'Plan a new activity',
                },
                {
                  label: 'Live Execution Teams',
                  value: liveActivities,
                  helper: `${projectActivities.length} total work items tracked`,
                },
                {
                  label: 'Completed Work',
                  value: completedActivities,
                  helper: `${plannedActivities} still in planning`,
                },
              ].map(item => (
          ...
          <div style={{ ...card, padding: 18 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: 'var(--text)', marginBottom: 8 }}>Planning Note</div>
            <div style={{ fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: 14 }}>
              Use modal-based activity planning to keep execution records clean and consistent across teams.
            </div>
            ...
            {projectActivities.map(item => (
              <div key={item.id} style={{ ...softCard, padding: '16px 18px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  <Space wrap size={[8, 8]}>
                    <Button type="link" size="small" onClick={() => openActivityCrudModal('view', item)} style={{ padding: 0 }}>View</Button>
                    <Button type="link" size="small" onClick={() => openActivityCrudModal('edit', item)} style={{ padding: 0 }}>Edit</Button>
                    <Button
                      type="link"
                      size="small"
                      danger
                  ...
                  <Button onClick={() => handleAdvanceActivityStatus(item.id)} style={secondaryButtonStyle}>
                    Advance Status
                  </Button>
                </div>
              </div>
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
          {[
            { label: 'Approved Quote', value: formatMoney(approvedAmount) },
            { label: 'Total Invoiced', value: formatMoney(totalInvoiced) },
            { label: 'Collections Received', value: formatMoney(amountReceived) },
            { label: 'Balance Outstanding', value: formatMoney(balanceOutstanding) },
            { label: 'Next Due', value: formatMoney(scheduledAmount || PROJECT_FINANCIALS.nextDueAmount) },
          ].map(item => (
            <div key={item.label} style={{ ...card, padding: 16 }}>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 6 }}>{item.label}</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)' }}>{item.value}</div>
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: isFinancialCompact ? '1fr' : 'minmax(0, 1.5fr) minmax(320px, 0.9fr)',
            gap: 16,
            alignItems: 'start',
          }}
        >
